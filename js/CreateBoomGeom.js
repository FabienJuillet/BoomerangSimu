						...
						if (childEl.arrow!==undefined)
							{
								...
								childEl.arrow.traverse(function (child) {
															if ((child instanceof THREE.Mesh)&&(child.name!=undefined))
															{
																if (child.name=="cylinder") {cylinderMesh=child;};
																if (child.name=="spear") {spearMesh=child;};								
...
function UpdateBoomerangInitialConditions(){
	...
	//boomerangObject.Velocity= new THREE.Vector3();//
	boomerangObject.Velocity= new THREE.Vector3(30,30*Math.tan(AngleHor*Math.PI/180),0);
	boomerangObject.RotationVector= new THREE.Vector3(0,0,-60);
